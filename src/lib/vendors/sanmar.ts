...
export async function getSanmarCost(
  ...
  try {
    ...
    for (const rec of candidates) {
      // Prefer myPrice (customer-negotiated net), fall back to casePrice, then piecePrice
      const cost = parsePrice(rec.myPrice) ?? parsePrice(rec.casePrice) ?? parsePrice(rec.piecePrice)
